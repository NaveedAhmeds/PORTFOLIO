...
const quoteVariants = {
  ...
  visible: { opacity: 1, x: 0, transition: { type: "spring", stiffness: 50, damping: 12, duration: 0.8 } },
};
      ...
      {experiences.map((exp, i) => {
        ...
        return (
          <motion.article
            ...
            whileHover={{ rotateX: -10, rotateY: isLeft ? 10 : -10, scale: 1.05, transition: { type: "spring", stiffness: 300, damping: 20 } }}
            whileTap={{ scale: 0.95 }}
          >
            {/* Dot */}
            <div
            ...
            >
              <header style={{ display: "flex", alignItems: "center", gap: "1rem", marginBottom: "0.75rem", flexWrap: "wrap" }}>
                <div style={{ backgroundColor: exp.color + "33", padding: "0.4rem", borderRadius: "0.5rem", display: "flex", alignItems: "center", justifyContent: "center", width: 36, height: 36 }}>
                  <Icon size={20} color={exp.color} />
                </div>
                <div>
                  <h4 style={{ margin: 0, fontWeight: "700", color: "white", fontFamily: "'Cinzel', serif", fontSize: "1.2rem" }}>{exp.title}</h4>
                  <p style={{ margin: 0, fontWeight: "600", color: exp.color, fontFamily: "'Neue Montreal', sans-serif", fontSize: "0.85rem" }}>
                    {exp.company} &bull; {exp.type}
                  </p>
                </div>
              </header>
              <p style={{ color: "rgba(255,255,255,0.8)", fontFamily: "'Neue Montreal', sans-serif", fontSize: "0.95rem", marginBottom: "0.5rem" }}>{exp.description}</p>
              <ul style={{ margin: 0, paddingLeft: "1rem", color: "rgba(255,255,255,0.7)", fontSize: "0.85rem", fontFamily: "'Neue Montreal', sans-serif", listStyleType: "disc", lineHeight: 1.3 }}>
            ...
            <motion.div
              ...
              style={{
                ...
                boxShadow: isLeft ? "4px 4px 10px rgba(0,0,0,0.2)" : "-4px 4px 10px rgba(0,0,0,0.2)",
